<?php
  include('session.php');
   ...
   $receipt_sql = "SELECT booking.bookingID, booking.cost, user.userID, user.name, user.childName, camp.campID, camp.name AS campName, camp.startdate, camp.enddate FROM booking INNER JOIN user ON booking.userID = user.userID INNER JOIN camp ON booking.campID = camp.campID WHERE booking.bookingID ='$_SESSION[login_user]'";
   $receipt_result = mysqli_query($conn,$receipt_sql);
   print_r($receipt_result);
  ...
  <div class="container">
    ...
    <div class="d-flex justify-content-center">
        ...
        <?php
          ...
          echo "You (" . $receipt_row['name'] . ") have booked your child " . $receipt_row['childName'] . " into " . $receipt_row['campName'] . " between the dates " . $receipt_row['startdate'] . " and " . $receipt_row['enddate'];
      
          echo "<br>The total cost is: £" . $receipt_row['cost'];
